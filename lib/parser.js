...
Parser.parseMlsdListEntry = function(line){
  var ret = {};

  var nameMatch = line.match(REX_MLSD_NAME);
  if(nameMatch) {
    ret.name = nameMatch[1];

    var params = line.split(';');
    for (var i = 0; i < params.length; i++) {
      var paramMatch = params[i].match(REX_MLSD_PARAM);
      if (paramMatch && paramMatch.length >= 3) {
        switch (paramMatch[1].toLowerCase()) {
          case 'size':
            ret.size = parseInt(paramMatch[2]);
            break;
          case 'type':
            ret.type = paramMatch[2];
            break;
          case 'modify':
            var dateval = XRegExp.exec(paramMatch[2], REX_TIMEVAL);
            if (dateval) {
              ret.date = new Date(Date.UTC(dateval.year, parseInt(dateval.month-1), dateval.date, dateval.hour, dateval.minute, dateval.second))
            }
            break;
        }
      }
